            ...
            { // NON-NLS
                ...
                else
                {
                    ...
                    if (realRegion == null)
                    {
                        if (realWorld == null)
                        {
                            sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.not.found"), region));
                        }
                        else
                        {
                            sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.in.world.not.found"), region, world));
                        }
                    }
                    else
                    {
                        if (realRegion.getTemplate() instanceof OwnableTemplate)
                        {
                            ...
                            if (ot.isRegionOccupied(realRegion))
                            {
                                ...
                                sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.in.world.released"), region, world));
                                ...
                                catch (ContentSaveException e)
                                {
                                    ...
                                    SimpleRegionMarket.getInstance().getLogger().severe(MessageFormat.format(LanguageSupport.instance.getString("region.save.problem.console"), realRegion.getName()));
                                    SimpleRegionMarket.getInstance().printError(e);
                                }
                            }
                            else
                            {
                                sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.in.world.already.free"), region, world));
                            }
                        }
                        else
                        {
                            sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.in.world.cannot.be.owned"), region, world));
                        }
                    }
                }
            }
                ...
                { // NON-NLS
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            sender.sendMessage(MessageFormat.format(LanguageSupport.instance.getString("region.in.world.removed"), region, world));
                        }
                    }
                }
                else
                    ...
                    { // NON-NLS
                        ...
                        if (!isConsole)
                        {
                            ArrayList<Region> playerRegions = SimpleRegionMarket.getPlugin().getPlayerManager().getPlayerRegions((Player) sender);
                        ...
                        if (region == null)
                        {
                            if (args.length > 2)
                            {
                                skip = 2;
                                regionString = args[1];
                                region = SimpleRegionMarket.getPlugin().getWorldHelper().getRegion(regionString, null);
                                if (region == null && !isConsole)
                                {
                                    region = SimpleRegionMarket.getPlugin().getWorldHelper().getRegion(regionString, player.getWorld());
                                }
                            }
                        }
