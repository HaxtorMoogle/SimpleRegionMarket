...
public class WorldEditManager
{
     ...
     */
    public WorldEditPlugin getWorldEdit() throws MoogleGenericException
    {
        Plugin worldEdit = Bukkit.getServer().getPluginManager().getPlugin("WorldEdit");
        if (worldEdit == null)
        {
            throw new MoogleGenericException("WorldEdit does not appear to be installed.");
        }
        ...
        {
            throw new MoogleGenericException("WorldEdit detection failed (report error).");
        }
    }

    public File getSchematicFile(RegionFactory.Region region)
    {
        return new File(new File(new File(new File(SimpleRegionMarket.getPlugin().getDataFolder(), TemplateManager.REGIONS_FOLDER), region.getTemplate().getId().toLowerCase()), region.getWorld()
    ...
    public void replaceRegionFromSchematic(RegionFactory.Region region)
    {
        if (region.getWorldguardRegion() instanceof ProtectedCuboidRegion)
        {
            ...
            {
                SimpleRegionMarket.getPlugin().getLogger().warning(MessageFormat.format(LanguageSupport.instance.getString("region.schematic.load.failure"), schematicFile.getPath()));
                SimpleRegionMarket.getPlugin().printError(e);
                return;
            }
