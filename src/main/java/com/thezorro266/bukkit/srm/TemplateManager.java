...
public class TemplateManager
{
    ...
    // Needs LanguageSupport,WorldGuardManager,WorldEditManager, RegionFactory,  to be loaded
    
    @Getter
    private final List<Template> templateList = new ArrayList<Template>();
    
    public SimpleRegionMarket thePlugin;
    public TemplateManager(SimpleRegionMarket plug)
    {
        thePlugin = plug;
    }
        ...
        synchronized (templateList)
        {
            for (Template template : templateList)
            {
                if (template.getId().equalsIgnoreCase("SELL"))
                {
                    ...
                    if (template.getId().equalsIgnoreCase("HOTEL"))
                    {
        ...
        for (final String world : agentsYaml.getKeys(false))
        {
            ...
            for (final String region : path.getKeys(false))
            {
                ...
                for (final String signnr : path.getKeys(false))
                {
                    ...
                    if (path.getInt("Mode") == 1)
                    { // HOTEL

                        /*
                         * if (!tokenHotel.entries.containsKey(world)) {
                         * tokenHotel.entries.put(world, new HashMap<String,
                         * HashMap<String, Object>>()); } if
                         * (!tokenHotel.entries.get(world).containsKey(region))
                         * { tokenHotel.entries.get(world).put(region, new
                         * HashMap<String, Object>());
                         * Utils.setEntry(tokenHotel, world, region, "price",
                         * path.getInt("Price")); Utils.setEntry(tokenHotel,
                         * world, region, "account", path.getInt("Account"));
                         * Utils.setEntry(tokenHotel, world, region, "renttime",
                         * path.getLong("RentTime")); if
                         * (path.isSet("ExpireDate")) {
                         * Utils.setEntry(tokenHotel, world, region, "taken",
                         * true); Utils.setEntry(tokenHotel, world, region,
                         * "owner", path.getString("RentBy"));
                         * Utils.setEntry(tokenHotel, world, region,
                         * "expiredate", path.getLong("ExpireDate")); } else {
                         * Utils.setEntry(tokenHotel, world, region, "taken",
                         * false); } }
                         * 
                         * final ArrayList<Location> signLocations =
                         * Utils.getSignLocations(tokenHotel, world, region);
                         * signLocations.add(new Location(realWorld,
                         * path.getDouble("X"), path.getDouble("Y"),
                         * path.getDouble("Z"))); if (signLocations.size() == 1)
                         * { Utils.setEntry(tokenHotel, world, region, "signs",
                         * signLocations); }
                         */
                    }
                    else
                    { // SELL
                         ...
                         * tokenAgent.entries.put(world, new HashMap<String,
                         * HashMap<String, Object>>()); } if
                         * (!tokenAgent.entries.get(world).containsKey(region))
                         * { tokenAgent.entries.get(world).put(region, new
                         * HashMap<String, Object>());
                         * Utils.setEntry(tokenAgent, world, region, "price",
                         * path.getInt("Price")); Utils.setEntry(tokenAgent,
                         * world, region, "account", path.getInt("Account"));
                         * Utils.setEntry(tokenAgent, world, region, "renttime",
                         * path.getLong("RentTime")); Utils.setEntry(tokenAgent,
                         * world, region, "taken", false); }
                         * 
                         * final ArrayList<Location> signLocations =
                         ...
                         * path.getDouble("X"), path.getDouble("Y"),
                         * path.getDouble("Z"))); if (signLocations.size() == 1)
                         * { Utils.setEntry(tokenAgent, world, region, "signs",
                         * signLocations); }
                         */
                    }
                }
    ...
    private void update(File templateFile, YamlConfiguration templateYaml) throws TemplateFormatException, IOException
    {
        ...
        if (templateYaml.isSet("templates_version"))
        {
            for (String templateId : templateYaml.getRoot().getKeys(false))
            {
                ...
                if (templateYaml.isSet(templateId + ".bidtime"))
                {
                    ...
                    {
                        templateYaml.set(templateId + ".bidtime.min", templateYaml.getInt(templateId + ".bidtime.min") / 1000);
                    }
                    if (templateYaml.isSet(templateId + ".bidtime.max"))
                    {
                        templateYaml.set(templateId + ".bidtime.max", templateYaml.getInt(templateId + ".bidtime.max") / 1000);
                    }
                ...
                {
                    templateYaml.set(templateId + "renttime.min", templateYaml.getInt(templateId + ".renttime.min") / 1000);
                }
                if (templateYaml.isSet(templateId + ".renttime.max"))
                {
                    templateYaml.set(templateId + "renttime.max", templateYaml.getInt(templateId + ".renttime.max") / 1000);
                }
                ...
                if (templateYaml.isSet(templateId + ".output.occupied"))
                {
                    for (int i = 1; i <= SIGN_LINE_COUNT; ++i)
                    {
                        ...
                        if (type.equalsIgnoreCase("TemplateSell"))
                        {
                            ...
                            if (type.equalsIgnoreCase("TemplateLease"))
                            {
                                ...
                                if (type.equalsIgnoreCase("TemplateRent"))
                                {
                                    ...
                                    if (type.equalsIgnoreCase("TemplateAuction"))
                                    {
        ...
        // Very old
            ...
            // New
                ...
                if (templateYaml.getInt("version") != TEMPLATE_VERSION)
                {
            ...
            for (Template template : templateList)
            {
                ...
                if (templateDir.exists())
                {
                    for (String worldStr : templateDir.list())
                    {
                        ...
                        {
                            // Check the world and get the world object from
                            // Bukkit
                            ...
                            if (world != null)
                            {
                                ...
                                if (files != null)
                                {
                                    for (File regionFile : files)
                                    {
                                        ...
                                        // Open the region configuration file
                                        // and try to load it
                                        try
                                        {
    ...
    // synchronized (templateList) {
    // for (Template template : templateList) {
    // File templateDir = new File(new
    // File(SimpleRegionMarket.getInstance().getDataFolder(), REGIONS_FOLDER),
    ...
    // File regionFile = new File(worldFolder,
    // String.format(REGIONS_YML_FORMAT_STRING,
    // region.getName()));
